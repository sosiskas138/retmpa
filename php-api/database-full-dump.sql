-- =====================================================
-- ПОЛНЫЙ ДАМП БАЗЫ ДАННЫХ РОССИЙСКОГО ПРОДУКТОВОГО РИТЕЙЛА
-- =====================================================
-- Версия: 2.0
-- Дата: 2025-12-18
-- Описание: Полный дамп всех таблиц и данных
-- Таблицы: companies, founders, events, financials, ipo, ma_events, error_reports
-- =====================================================
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='Компании российского продуктового ритейла';
...
CREATE TABLE `founders` (
  ...
  INDEX idx_status (status)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='Основатели и ключевые лица компаний';
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='Ключевые события компаний';
...
-- =====================================================
-- ТАБЛИЦА 4: financials (Финансовые данные)
-- =====================================================
...
CREATE TABLE `financials` (
  ...
  `year` INT NOT NULL,
  `quarter` INT NULL,
  `revenue` DECIMAL(15,2) COMMENT 'Выручка в млрд руб',
  `profit` DECIMAL(15,2) COMMENT 'Чистая прибыль в млрд руб',
  `margin` DECIMAL(5,2) COMMENT 'Маржа чистой прибыли в %',
  `store_count` INT COMMENT 'Количество магазинов',
  `created_at` TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
  `updated_at` TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  UNIQUE KEY `unique_company_year_quarter` (`company_id`, `year`, `quarter`),
  INDEX idx_company (company_id),
  INDEX idx_year (year)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='Финансовые показатели компаний';

-- Финансовые данные Магнита
...
CREATE TABLE `ipo` (
  ...
  `date` DATE NOT NULL,
  `valuation` VARCHAR(100) COMMENT 'Оценка компании',
  `raised` VARCHAR(100) COMMENT 'Привлечено средств',
  `revenue` VARCHAR(100) COMMENT 'Выручка на момент размещения',
  `shares` TEXT COMMENT 'Размещённые акции',
  `exchange` VARCHAR(100) COMMENT 'Биржа',
  `ticker` VARCHAR(50) COMMENT 'Тикер',
  `price` VARCHAR(100) COMMENT 'Цена размещения',
  `prospectus` TEXT COMMENT 'Проспект эмиссии',
  `market_position` TEXT COMMENT 'Рыночная позиция',
  `created_at` TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
  `updated_at` TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  INDEX idx_company (company_id),
  INDEX idx_date (date),
  INDEX idx_type (type)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='IPO и SPO размещения';
...
-- =====================================================
-- ТАБЛИЦА 6: ma_events (M&A события)
-- =====================================================
...
CREATE TABLE `ma_events` (
  ...
  `target` VARCHAR(255) NOT NULL COMMENT 'Цель сделки',
  `value` VARCHAR(100) COMMENT 'Сумма сделки',
  `description` TEXT COMMENT 'Описание сделки',
  `created_at` TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
  `updated_at` TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  INDEX idx_date (date),
  INDEX idx_buyer (buyer)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='Сделки слияний и поглощений';
...
-- =====================================================
-- ТАБЛИЦА 7: error_reports (Сообщения об ошибках)
-- =====================================================
...
-- ПРОВЕРКА ИМПОРТА
-- =====================================================
SELECT '=== РЕЗУЛЬТАТ ИМПОРТА ===' as info;
SELECT 'companies' as `Таблица`, COUNT(*) as `Записей` FROM companies
UNION ALL SELECT 'founders', COUNT(*) FROM founders
UNION ALL SELECT 'events', COUNT(*) FROM events
UNION ALL SELECT 'financials', COUNT(*) FROM financials
UNION ALL SELECT 'ipo', COUNT(*) FROM ipo
UNION ALL SELECT 'ma_events', COUNT(*) FROM ma_events
UNION ALL SELECT 'error_reports', COUNT(*) FROM error_reports;

-- =====================================================
-- КОНЕЦ ПОЛНОГО ДАМПА БАЗЫ ДАННЫХ
-- =====================================================
